...
use self::io::{spawn_pty_reader_with_resize, spawn_stdin_reader};
use self::pty::PtyManager;
use self::terminal::{build_command, disable_raw_mode, enable_raw_mode, get_pty_size};
...
pub struct TerminalServer<T: Transport + Send + 'static> {
    ...
    delta_tx: Arc<
        tokio::sync::Mutex<
            Option<tokio::sync::mpsc::Sender<crate::server::terminal_state::GridDelta>>,
        >,
    >,
}
...
impl<T: Transport + Send + 'static> TerminalServer<T> {
    ...
    pub async fn create(
        ...
        let terminal_tracker = Arc::new(std::sync::Mutex::new(TerminalStateTracker::from_backend(
        ...
        if let Some(debug_log) = &mut *terminal_server.debug_log.lock().unwrap() {
            ...
            let _ = writeln!(
                debug_log,
                "[{}] Server: Created TrackerDataSource with delta channel",
        ...
        *terminal_server.delta_tx.lock().await = Some(delta_tx);
        ...
        // Wire PTY writer so input from clients is forwarded to the shell
        {
            ...
            subscription_hub
                .set_pty_writer(Arc::new(PtyWriterFromManager::new_with_debug(
        ...
        // Set the terminal server as the message handler directly
        {
            ...
            session
                .set_handler(terminal_server.clone() as Arc<dyn ServerMessageHandler>)
                .await;
        ...
        if let Some(debug_log) = &mut *terminal_server.debug_log.lock().unwrap() {
            ...
            let _ = writeln!(
                debug_log,
                "[{}] Server: Starting subscription hub streaming task",
                chrono::Local::now().format("%H:%M:%S%.3f")
            );
        }
    ...
    pub fn new_with_debug_recorder(
            ...
            Ok((cols, rows)) => {
                if let Some(ref log_file) = debug_log_file {
                    if let Ok(log_file) = log_file.try_clone() {
                        ...
                        let _ = writeln!(
                            log,
                            "[{}] TerminalServer::new detected terminal size: {}x{}",
            ...
            Err(_) => {
                if let Some(ref log_file) = debug_log_file {
                    if let Ok(log_file) = log_file.try_clone() {
                        ...
                        let _ = writeln!(
                            log,
                            "[{}] TerminalServer::new failed to detect terminal size, using defaults",
                            chrono::Utc::now().format("%H:%M:%S%.3f")
                        );
                    }
                }
                    ...
                    Err(e) => {
                        ...
                        if let Some(ref log_file) = debug_log_file {
                            if let Ok(log_file) = log_file.try_clone() {
                                ...
                                let _ = writeln!(
                                    log,
                                    "[{}] Failed to create debug recorder: {}",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    e
                                );
                            }
...
impl<T: Transport + Send + 'static> Server for TerminalServer<T> {
    ...
    async fn start(&self) {
        ...
        if let Some(debug_log) = self.debug_log.lock().unwrap().as_mut() {
            ...
            let _ = writeln!(
                debug_log,
                "[{}] Server::start detected terminal size: {}x{}",
                chrono::Utc::now().format("%H:%M:%S%.3f"),
                pty_size.cols,
                pty_size.rows
            );
            ...
            if let Ok((cols, rows)) = crossterm::terminal::size() {
                ...
                let _ = writeln!(
                    debug_log,
                    "[{}] Detection method: fallback (crossterm failed)",
                    chrono::Utc::now().format("%H:%M:%S%.3f")
                );
            }
        }
        ...
        let cmd = {
            ...
            if let Some(debug_log) = self.debug_log.lock().unwrap().as_mut() {
                ...
                let _ = writeln!(
                    debug_log,
                    "[{}] Server::start executing command: {:?}",
                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                    cmd_vec
                );
            }
        ...
        // Resize terminal backend to match current PTY size (in case it changed)
        {
            ...
            if current_width != pty_size.cols || current_height != pty_size.rows {
                ...
                if let Some(debug_log) = self.debug_log.lock().unwrap().as_mut() {
                    ...
                    let _ = writeln!(
                        debug_log,
                        "[{}] Server::start resizing backend from {}x{} to {}x{}",
                        chrono::Utc::now().format("%H:%M:%S%.3f"),
                        current_width,
                        current_height,
                        pty_size.cols,
                ...
                if let Some(debug_log) = self.debug_log.lock().unwrap().as_mut() {
                    ...
                    let _ = writeln!(
                        debug_log,
                        "[{}] Server::start backend already at correct size {}x{}",
                        chrono::Utc::now().format("%H:%M:%S%.3f"),
                        pty_size.cols,
                        pty_size.rows
                    );
        ...
        self.debug_handler
            .set_backend(Arc::clone(&self.terminal_backend));
        ...
        *self.read_task.lock().unwrap() = Some(read_task);
        ...
        let stdin_task =
            spawn_stdin_reader(self.pty_manager.master_writer.clone(), stdin_recorder_path);
...
impl<T: Transport + Send + 'static> TerminalServer<T> {
            ...
            if wait_for_webrtc {
                if session.has_any_webrtc_connected().await {
                    if let Some(ref mut debug_log) = *self.debug_log.lock().unwrap() {
                        ...
                        let _ = writeln!(
                            debug_log,
                            "[{}] Server: WebRTC connection established",
                            chrono::Utc::now().format("%H:%M:%S%.3f")
                        );
                    }
                    break;
    ...
    pub async fn start_with_wait(
        ...
        if let Some(ref mut debug_log) = *self.debug_log.lock().unwrap() {
            ...
            let _ = writeln!(
                debug_log,
                "[{}] Server: start_with_wait called - wait_for_client={}, wait_for_webrtc={}, keep_alive={}",
                chrono::Utc::now().format("%H:%M:%S%.3f"),
                wait_for_client,
                wait_for_webrtc,
                keep_alive
        ...
        if keep_alive {
            ...
            if let Some(ref mut debug_log) = *self.debug_log.lock().unwrap() {
                ...
                let _ = writeln!(
                    debug_log,
                    "[{}] Server: Entering keep-alive mode",
                    chrono::Utc::now().format("%H:%M:%S%.3f")
                );
            }
        ...
        match message {
            AppMessage::Protocol { message: msg_value } => {
                ...
                if let Ok(client_msg) =
                    serde_json::from_value::<crate::protocol::ClientMessage>(msg_value)
                {
                    ...
                    match client_msg {
                        CMsg::Subscribe {
                            ...
                            if let Some(ref mut debug_log) = *self.debug_log.lock().unwrap() {
                                ...
                                let _ = writeln!(
                                    debug_log,
                                    "[{}] Server: Received Subscribe {{ id: {} }} from {}",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    subscription_id,
                                    from_peer
                                );
                            ...
                            if let Some(client_transport) = transport_ready {
                                let transport_dyn =
                                    client_transport as Arc<dyn crate::transport::Transport>;
                                let config = crate::subscription::SubscriptionConfig {
                                    dimensions,
                                    mode: mode.unwrap_or(crate::protocol::ViewMode::Realtime),
                                    position,
                                    is_controlling: true,
                                    initial_fetch_size, // From client Subscribe message
                                    stream_history,     // From client Subscribe message
                                };
                                drop(session);
                                let result = hub
                                    .subscribe_with_id(
                                ...
                                if let Some(ref mut debug_log) = *self.debug_log.lock().unwrap() {
                                    ...
                                    let _ = writeln!(
                                        debug_log,
                                        "[{}] Server: subscribe_with_id result: {:?}",
                                        chrono::Utc::now().format("%H:%M:%S%.3f"),
