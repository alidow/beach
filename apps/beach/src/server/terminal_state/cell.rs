...
impl Cell {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, TerminalStateError> {
        ...
        let char = char_str.chars().next()
            .ok_or_else(|| TerminalStateError::SerializationError(
                "Empty char string".to_string()
            ))?;
        cursor += char_len;
...
impl CellAttributes {
    fn to_byte(&self) -> u8 {
        let mut byte = 0u8;
        if self.bold { byte |= 1 << 0; }
        if self.italic { byte |= 1 << 1; }
        if self.underline { byte |= 1 << 2; }
        if self.strikethrough { byte |= 1 << 3; }
        if self.reverse { byte |= 1 << 4; }
        if self.blink { byte |= 1 << 5; }
        if self.dim { byte |= 1 << 6; }
        if self.hidden { byte |= 1 << 7; }
        byte
    }
