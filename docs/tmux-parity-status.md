# beach tmux Parity: Status & Next Steps

 _Last updated: 2025-01-07 (afternoon)_

## Update (2025-01-07)
- ✅ **Step 1 — Mouse pipeline**: beach now enables crossterm mouse reporting, handles wheel input locally, and forwards ANSI SGR mouse frames to the host. Copy mode auto-engages on upward scroll and drops out when the viewport snaps back to tail, matching tmux's feel.
...
- **What we do today**: As soon as the snapshot completes we aggressively request history (e.g. `start=33`, `start=154`) even while following the live tail. The host often responds with empty chunks, which our renderer treats as authoritative; it advances `base_row` past the missing span and leaves the freshly pushed rows wedged above a wall of `Pending` placeholders. We then keep re-requesting the same empty ranges and never show the full burst. Worse, because the handshake `Grid` frame tells the client to pre-allocate the entire advertised history, every unseen row is marked `Pending` before the renderer ever draws it. The gap detector treats those placeholders as real gaps and immediately fires `maybe_request_backfill`, so tail-follow mode starts requesting backfills (`start=26 count=44` in the Sep 23 burst) even though the user never scrolled.
...
- Added `client_marks_empty_backfill_as_missing` (`apps/beach/tests/client_transcripts.rs:878`) to capture the empty-tail wedge; it failed before the fix because the client immediately re-requested the same range.
...
- **Grid height advertised as total history length** – The host sends `HostFrame::Grid { viewport_rows, cols, history_rows }` using `terminal_sync.grid().viewport_size()` and `TerminalGrid::rows()` (`apps/beach/src/main.rs:1769`). Earlier revisions used `TerminalGrid::dims()` (all buffered history) for the `rows` field, so the client seeded `GridRenderer::ensure_size(rows, cols)` with a massive value and `scroll_to_tail()` (`apps/beach/src/client/grid_renderer.rs:604`) immediately jumped past the freshly streamed tail slice.  
  **Proposed fix**: teach the server to advertise the actual viewport height. `AlacrittyEmulator::render_full_internal` already knows `term_grid.screen_lines()`. Either (a) thread the emulator's `screen_lines` through `TerminalGrid` so `dims()` can return `(screen_lines, cols)`, or (b) override the rows field in the handshake with the PTY window height we already fetch during `process.resize(cols, rows)` (`apps/beach/src/main.rs:1087`). Clients should keep tracking absolute rows, but the initial `rows` needs to be "visible rows" so the viewport math stays sane.

- **Follow-tail gap detector eagerly backfills** – Because we pre-mark unseen rows as `Pending`, the first call to `maybe_request_backfill` while `renderer.is_following_tail()` decides there is a gap and issues tail backfills despite the user staying at the live prompt. Empty replies leave those rows `Pending`, so the UI turns into a permanent wall of dots.
  **Proposed fix**: when `follow_tail` is active and no manual scroll-back has occurred, skip gap-triggered history requests entirely and rely on streamed deltas. Only once the user scrolls up (disabling follow-tail) should we scan for gaps and issue backfills. Longer term we can defer allocating history rows until the user needs them.

## CRITICAL: Absolute Row Mismatch After Host Attach

**Symptom:** After joining a brand-new session the client often shows a tall band of `Pending` rows (`···`) between the handshake snapshot and the live tail. No amount of following the tail clears the gap.

**Root cause:** The server and client both index the scrollback using the PTY's absolute row IDs. When the host shell starts it frequently *restores existing scrollback* before beach spawns (macOS Terminal prints `Restored session: …`, many shells replay prompt history, etc.). Those lines populate the PTY buffer before any new commands run. As a result, the first line produced after beach starts can land at row 80, 120, or even higher. The initial snapshot only contains the visible viewport (rows 0–23), so rows 24–n are left uninitialised (`Pending`). The server emits deltas for rows 120+, the client applies them, and the gap in between never receives data.

**Why this is CRITICAL:** The behaviour depends on the host terminal emulator, shell configuration, and whatever command was running beforehand. We cannot require users to manually clear scrollback, adjust dotfiles, or limit themselves to a particular shell. Unless we normalise the row numbering ourselves, any environment that restores history will hit the same mismatch and the client will wedge on `Pending` rows.

**Requirements for a fix:**

1. The server must provide the client with a *relative base row* that reflects the first printable line of the session, regardless of the PTY's internal absolute counter.
2. Alternatively, the server must stream the entire restored scrollback during the handshake so the client can populate those rows instead of marking them `Pending`.
3. The solution must work without shell-specific hacks (no `printf '\033c'` expectations, no zsh plugin tweaks) and must not rely on the user clearing scrollback manually.

Until we satisfy at least one of the above, tmux parity remains blocked—the joiner will always encounter false gaps whenever the host PTY comes with preloaded history.

- **Local banners never reach the replicated grid** – The host prints `print_host_banner(...)` (`apps/beach/src/main.rs:557`) *before* the PTY is live, so "🏖️ beach session ready!" and the share command never enter the grid cache. The joiner prints its own "🌊 Joined session" banner (`apps/beach/src/main.rs:574`) locally, also outside the synchronized stream. When we compare host/guest transcripts the row counts are offset by those local-only banners, which makes it appear as if the client missed early lines.  
