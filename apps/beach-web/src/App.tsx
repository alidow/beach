...
export default function App(): JSX.Element {
  ...
  const computeDockedPosition = (dock: 'top' | 'bottom', metrics?: LayoutMetrics): Position => {
    ...
    const { shellRect, panelSize } = layout;
    const baseTop = dock === 'top' ? DOCK_MARGIN : shellRect.height - panelSize.height - DOCK_MARGIN;
  ...
  const dockToggleLabel =
    dockPreference === 'top' ? 'Move connection info to bottom' : 'Move connection info to top';
    ...
    >
      ...
      {infoVisible ? (
        <div className={toolbarWrapperClass} style={toolbarWrapperStyle}>
          <div ref={toolbarRef} className="pointer-events-auto min-w-[18rem] w-full max-w-3xl sm:w-auto">
            <Collapsible
              open={infoExpanded}
              onOpenChange={(open) => {
                if (status !== 'connected') {
                      ...
                      >
                        ...
                        <ChevronDown
                          className={cn(
                            ...
                            infoExpanded ? 'rotate-180' : 'rotate-0',
                          )}
                        />
                      </Button>
                    </CollapsibleTrigger>
      ...
      <Dialog open={showModal} onOpenChange={() => {}}>
        <DialogContent className="w-[min(94vw,420px)] max-h-[min(92svh,520px)] overflow-y-auto p-7 sm:w-[440px] sm:p-8">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription>
              Enter the session credentials from your host to begin streaming the terminal. Everything happens live in
              this window.
            </DialogDescription>
          </DialogHeader>
          <form
