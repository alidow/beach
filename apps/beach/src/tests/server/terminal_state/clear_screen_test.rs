...
fn test_clear_to_end_of_screen() {
    ...
    for row in 0..5 {
        let line = (0..5).map(|col| {
            grid_before.get_cell(row, col)
...
fn test_clear_after_wrap() {
    ...
    println!("After wrap:");
    println!("  Cursor: row={}, col={}", grid_after_wrap.cursor.row, grid_after_wrap.cursor.col);
    for row in 0..3 {
        let line = (0..10).map(|col| {
            grid_after_wrap.get_cell(row, col)
                .map(|c| if c.char == ' ' || c.char == '\0' { '_' } else { c.char })
                .unwrap_or('?')
        }).collect::<String>();
        println!("  Row {}: '{}'", row, line);
    ...
    println!("  Cursor: row={}, col={}", grid_after_clear.cursor.row, grid_after_clear.cursor.col);
    for row in 0..3 {
        let line = (0..10).map(|col| {
            grid_after_clear.get_cell(row, col)
                .map(|c| if c.char == ' ' || c.char == '\0' { '_' } else { c.char })
                .unwrap_or('?')
        }).collect::<String>();
        println!("  Row {}: '{}'", row, line);
    ...
    for row in 0..3 {
        for col in 0..10 {
            ...
            assert!(cell.char == ' ' || cell.char == '\0', 
                    "Cell at ({}, {}) should be cleared, but has '{}'", row, col, cell.char);
        }
    }
}