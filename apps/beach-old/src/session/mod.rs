...
use self::message_handlers::{ClientMessageHandler, ServerMessageHandler};
use self::signaling_transport::{SignalingTransport, create_websocket_signaling};
use crate::config::Config;
use crate::protocol::signaling::{
    AppMessage, ClientMessage, PeerRole, ServerMessage, TransportSignal, WebRTCSignal,
};
...
impl<T: Transport + Send + 'static> ServerSession<T> {
    ...
    pub fn set_debug_handler(
        &mut self,
        debug_handler: Arc<crate::server::debug_handler::DebugHandler>,
    ) {
                    ...
                    match msg {
                        ServerMessage::PeerJoined { peer } => {
                            ...
                            webrtc_channels
                                .write()
                                .await
                                .insert(peer.id.clone(), channel_arc.clone());

                            // Create per-client WebRTC transport
                            {
                                use crate::transport::TransportMode;
                                use crate::transport::webrtc::{
                                    WebRTCTransport, config::WebRTCConfigBuilder,
                                };
                                ...
                                match WebRTCTransport::new(config).await {
                                    Ok(client_transport) => {
                                        ...
                                        // Store the transport
                                        webrtc_transports_clone
                                            .write()
                                            .await
                                            .insert(peer_id.clone(), client_transport_arc.clone());

                                        // Take the incoming receiver for routing
                                        if let Some(mut rx) =
                                            client_transport_arc.take_incoming().await
                                        {
                                            // Start router task for this client
                                            tokio::spawn(async move {
                                                while let Some(bytes) = rx.recv().await {
                                                    if let Ok(app_msg) =
                                                        serde_json::from_slice::<AppMessage>(&bytes)
                                                    {
                                        ...
                                        if let Some(debug_log_path) = get_debug_log_path() {
                                                ...
                                                .open(&debug_log_path)
                                            {
                                                ...
                                                let _ = writeln!(
                                                    file,
                                                    "[{}] Server initiating WebRTC as offerer for client {}",
                                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                                    peer_id_for_log
                                                );
                                            }
                                                ...
                                                Ok(Ok(())) => {
                                                        ...
                                                        get_debug_log_path()
                                                    {
                                                                ...
                                                                .open(&debug_log_path)
                                                        {
                                                            ...
                                                            let _ = writeln!(
                                                                file,
                                                                "[{}] Server WebRTC connection established for client {}",
                                                                chrono::Utc::now()
                                                                    .format("%H:%M:%S%.3f"),
                                                                peer_id_for_log
                                                            );
                                                        ...
                                                        get_debug_log_path()
                                                    {
                                                                ...
                                                                .open(&debug_log_path)
                                                        {
                                                            ...
                                                            let _ = writeln!(
                                                                file,
                                                                "[{}] Server WebRTC ready for client {}, waiting for Subscribe message",
                                                                chrono::Utc::now()
                                                                    .format("%H:%M:%S%.3f"),
                                                                peer_id_for_log
                                                            );
                                                ...
                                                Ok(Err(e)) => {
                                                        ...
                                                        get_debug_log_path()
                                                    {
                                                                ...
                                                                .open(&debug_log_path)
                                                        {
                                                            ...
                                                            let _ = writeln!(
                                                                file,
                                                                "[{}] Server WebRTC connection failed for client {}: {}",
                                                                chrono::Utc::now()
                                                                    .format("%H:%M:%S%.3f"),
                                                                peer_id_for_log,
                                                                e
                                    ...
                                    Err(e) => {
                                        eprintln!(
                                            "Failed to create WebRTC transport for client {}: {}",
                                            peer.id, e
                                        );
                                    }
                                }
                                            ...
                                            && signal_type == "debug_request"
                                        {
                                                ...
                                                (debug_handler.as_ref(), signal.get("payload"))
                                            {
                                                if let Some(request) = payload.get("request") {
                                                    ...
                                                    let response = debug_handler
                                                        .handle_debug_request(request.clone())
                                                        .await;
        ...
        if strict_webrtc {
            ...
            if let Some(client_transport) = transports.get(client_id) {
                if client_transport.is_connected() {
                    ...
                    let is_terminal_data = if let AppMessage::Protocol {
                        message: ref msg_value,
                    } = message
                    {
                        if let Ok(server_msg) =
                            serde_json::from_value::<ServerMessage>(msg_value.clone())
                        {
                            matches!(
                                server_msg,
                    ...
                    if is_terminal_data {
                        ...
                        return channel.send(&bytes).await.map_err(|e| {
                            anyhow::anyhow!(
                                "WebRTC Output channel send to client {} failed: {}",
                                client_id,
                                e
                            )
                        });
                    } else {
                        return client_transport.send(&bytes).await.map_err(|e| {
                            anyhow::anyhow!(
                                "WebRTC Control channel send to client {} failed: {}",
                                client_id,
                                e
                            )
                        });
                    ...
                    return Err(anyhow::anyhow!(
                        "WebRTC transport not connected for client {}",
                        client_id
                    ));
                }
            } else {
                return Err(anyhow::anyhow!(
                    "No WebRTC transport found for client {}",
                    client_id
                ));
            }
        }
    ...
    pub async fn get_webrtc_transport(
        &self,
        client_id: &str,
    ) -> Option<Arc<crate::transport::webrtc::WebRTCTransport>> {
        let transports = self.webrtc_transports.read().await;
...
impl<T: Transport + Send + 'static> ClientSession<T> {
    ...
    async fn start_webrtc_receive_loop(&self, handler: Arc<dyn ClientMessageHandler>) {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let lock_result =
                    tokio::time::timeout(std::time::Duration::from_millis(5), transport.lock())
                        .await;
                ...
                let recv_result =
                    tokio::time::timeout(std::time::Duration::from_millis(20), recv_fut).await;
                        ...
                        ServerMessage::Signal { from_peer, signal } => {
                            ...
                            if let Some(debug_log_path) = get_debug_log_path() {
                                    ...
                                    .open(&debug_log_path)
                                {
                                    ...
                                    let _ = writeln!(
                                        file,
                                        "[{}] Client processing Signal from peer: {}",
                                        chrono::Utc::now().format("%H:%M:%S%.3f"),
                                        from_peer
                                    );
                                }
                            ...
                            if let Ok(transport_signal) = TransportSignal::from_value(&signal) {
                                ...
                                if matches!(webrtc_signal, WebRTCSignal::Offer { .. }) {
                                    ...
                                    if transport_guard.is_webrtc() && webrtc_channel.is_some() {
                                        ...
                                        let channel_any = channel_arc.clone()
                                        ...
                                        if let Some(debug_log_path) = get_debug_log_path() {
                                                ...
                                                .open(&debug_log_path)
                                            {
                                                ...
                                                let _ = writeln!(
                                                    file,
                                                    "[{}] Client initiating WebRTC as answerer",
                                                    chrono::Utc::now().format("%H:%M:%S%.3f")
                                                );
                                            }
                                        }
                                        ...
                                        tokio::spawn(async move {
                                            ...
                                            match result {
                                                Ok(Ok(())) => {
                                                        ...
                                                        get_debug_log_path()
                                                    {
                                                                ...
                                                                .open(&debug_log_path)
                                                        {
                                                            ...
                                                            let _ = writeln!(
                                                                file,
                                                                "[{}] Client WebRTC initialization successful",
                                                                chrono::Utc::now()
                                                                    .format("%H:%M:%S%.3f")
                                                            );
                                                        }
                                                ...
                                                Ok(Err(e)) => {
                                                        ...
                                                        get_debug_log_path()
                                                    {
                                                                ...
                                                                .open(&debug_log_path)
                                                        {
                                                            ...
                                                            let _ = writeln!(
                                                                file,
                                                                "[{}] Client WebRTC initialization failed: {}",
                                                                chrono::Utc::now()
                                                                    .format("%H:%M:%S%.3f"),
                                                                e
                                                            );
                                                    ...
                                                    {
                                                        panic!(
                                                            "WebRTC connection required but failed: {}. Ensure both peers support WebRTC and network allows peer-to-peer connections.",
                                                            e
                                                        );
                                                    }
                                                }
                                                Err(_) => {
                                                    ...
                                                    {
                                                        panic!(
                                                            "WebRTC handshake timed out after 30 seconds. This may indicate: 1) Network firewall blocking WebRTC, 2) Server not responding, 3) STUN/TURN servers unreachable"
                                                        );
                                                    }
                                                }
                                            }
    ...
    pub async fn send_to_server(&self, message: AppMessage) -> Result<()> {
        ...
        if strict_webrtc {
            ...
            let is_input_control = if let AppMessage::Protocol {
                message: ref msg_value,
            } = message
            {
            ...
            if let Some(debug_log_path) = get_debug_log_path() {
                    ...
                    .open(&debug_log_path)
                {
                    ...
                    let _ = writeln!(
                        file,
                        "[{}] ClientSession: Starting transport.send() for {} bytes ({})",
                    ...
                    .open(&debug_log_path)
                {
                    ...
                    match &result {
                        Ok(_) => {
                            let _ = writeln!(
                                file,
                                "[{}] ClientSession: transport.send() completed in {}μs",
                                chrono::Local::now().format("%H:%M:%S%.3f"),
                                elapsed.as_micros()
                            );
                            if elapsed.as_millis() > 5 {
                                let _ = writeln!(
                                    file,
                                    "[{}] ClientSession: WARNING: transport.send() took >5ms ({}ms)",
                                    chrono::Local::now().format("%H:%M:%S%.3f"),
                                    elapsed.as_millis()
                                );
                            }
                        ...
                        Err(e) => {
                            let _ = writeln!(
                                file,
                                "[{}] ClientSession: transport.send() failed after {}μs: {:?}",
                                chrono::Local::now().format("%H:%M:%S%.3f"),
                                elapsed.as_micros(),
                                e
                            );
...
impl<T: Transport + Send + 'static> Session<T> {
    ...
    pub async fn create(
        ...
        if let Err(e) = server_session
            .connect_signaling(&config.session_server, &session_id)
            .await
        {
    ...
    pub async fn join(
        ...
        transport: T,
        passphrase: Option<String>,
    ) -> Result<(ClientSession<T>, String, String)> {
        ...
        let (server_addr, session_id) = if parts.len() == 2 {
            ...
            return Err(anyhow::anyhow!(
                "Invalid session URL format. Expected: server/session_id"
            ));
        };
        ...
        session_client
            .join_session(session_id, passphrase.as_deref())
            .await
            .map_err(|e| anyhow::anyhow!("Failed to join session: {}", e))?;
