...
mod tests {
    use crate::transport::webrtc::{
        WebRTCTransport, config::WebRTCConfig, signaling::LocalSignalingChannel,
    };
    ...
    async fn test_webrtc_echo() {
        ...
        let received_1 = timeout(Duration::from_secs(1), client_transport.recv())
            .await
        ...
        let received_2 = timeout(Duration::from_secs(1), server_transport.recv())
            .await
        ...
        let received_large = timeout(Duration::from_secs(2), client_transport.recv())
            .await
    ...
    async fn test_webrtc_multiple_messages() {
        ...
        for _ in 0..num_messages {
            let msg = timeout(Duration::from_secs(1), client_transport.recv())
                .await
