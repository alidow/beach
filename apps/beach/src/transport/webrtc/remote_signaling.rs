...
use crate::protocol::signaling::{ClientMessage, TransportSignal, WebRTCSignal};
...
impl RemoteSignalingChannel {
    ...
    pub fn new(signaling: Arc<SignalingTransport<WebSocketTransport>>, peer_id: String) -> Self {
    ...
    pub async fn wait_for_answer(&self) -> Result<RTCSessionDescription> {
        ...
        while let Some(signal) = rx.recv().await {
            if let TransportSignal::WebRTC {
                signal: WebRTCSignal::Answer { sdp },
            } = signal
            {
                if std::env::var("BEACH_VERBOSE").is_ok() {
