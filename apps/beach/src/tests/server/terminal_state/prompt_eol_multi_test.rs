...
fn test_multiple_commands_with_prompt_eol() {
    ...
    assert!(grid.get_cell(0, 0).unwrap().char == '(' || grid.get_cell(0, 0).unwrap().char == ' ', 
    ...
    assert!(blank_lines <= 5, "Too many blank lines - likely PROMPT_EOL_MARK issue");
}