...
impl<T: Transport + Send + Sync + 'static> TerminalClient<T> {
    ...
    async fn handle_mouse_event(&self, mouse: crossterm::event::MouseEvent) -> Result<()> {
        ...
        if let Some(ref debug_log) = self.debug_log {
            if let Ok(mut file) = std::fs::OpenOptions::new().create(true).append(true).open(debug_log) {
                use std::io::Write;
                let _ = writeln!(file, "[{}] MouseEvent: kind={:?}, column={}, row={}",
                    chrono::Utc::now().format("%H:%M:%S%.3f"),
        ...
        match mouse.kind {
            MouseEventKind::ScrollUp => {
                ...
                if let Some(ref debug_log) = self.debug_log {
                    if let Ok(mut file) = std::fs::OpenOptions::new().create(true).append(true).open(debug_log) {
                        use std::io::Write;
                        let _ = writeln!(file, "[{}] ScrollUp: About to call scroll_vertical(3)",
                            chrono::Utc::now().format("%H:%M:%S%.3f"));
                    }
                }
                ...
                if let Some(ref debug_log) = self.debug_log {
                    if let Ok(mut file) = std::fs::OpenOptions::new().create(true).append(true).open(debug_log) {
                        ...
                        let metadata_info = if let Some(ref metadata) = renderer.history_metadata {
                            format!("metadata(oldest={}, latest={})", metadata.oldest_line, metadata.latest_line)
                        } else {
                            "no_metadata".to_string()
                        };
                        let _ = writeln!(file, "[{}] ScrollUp: After scroll_vertical, offset={}, grid_end_line={}, {}, view_line={:?}",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
            ...
            MouseEventKind::ScrollDown => {
                ...
                if let Some(ref debug_log) = self.debug_log {
                    if let Ok(mut file) = std::fs::OpenOptions::new().create(true).append(true).open(debug_log) {
                        use std::io::Write;
                        let _ = writeln!(file, "[{}] ScrollDown: About to call scroll_vertical(-3)",
                            chrono::Utc::now().format("%H:%M:%S%.3f"));
                    }
                }
                ...
                if let Some(ref debug_log) = self.debug_log {
                    if let Ok(mut file) = std::fs::OpenOptions::new().create(true).append(true).open(debug_log) {
                        ...
                        let _ = writeln!(file, "[{}] ScrollDown: After scroll_vertical, offset={}, grid_end_line={}, {}, view_line={:?}",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            renderer.scroll_offset,
                            renderer.grid.end_line,
                            metadata_info,
