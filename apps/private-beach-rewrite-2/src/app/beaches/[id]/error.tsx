...
export default function BeachError({ error, reset }: Props) {
  useEffect(() => {
    console.error('[ws-b] beach detail error boundary', error);
  }, [error]);

  return (
    <div className="flex min-h-screen flex-col bg-background">
      <AppShellTopNav backHref="/beaches" title="Private Beach" subtitle="Something went wrong loading this beach." />
      <main className="flex-1">
        <div className="mx-auto flex h-full max-w-3xl flex-col items-center justify-center gap-4 px-4 text-center sm:px-6 lg:px-8">
          <div className="rounded-lg border border-destructive/40 bg-destructive/10 px-6 py-4 text-sm text-destructive-foreground">
            {error.message || 'An unexpected error occurred while loading this beach.'}
          </div>
          <button
            type="button"
