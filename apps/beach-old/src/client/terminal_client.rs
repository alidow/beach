...
use crate::protocol::{
    ClientMessage, ControlMessage, Dimensions, ServerMessage, subscription::messages::Prefetch,
};
...
impl<T: Transport + Send + Sync + 'static> TerminalClient<T> {
    ...
    pub async fn create(
        ...
        // Join the session
        let (client_session, server_addr, session_id) =
            Session::join(session_str, transport, passphrase).await?;
        ...
        // Now connect signaling (router starts here because handler is already set)
        {
            ...
            if let Err(_e) = session
                .write()
                .await
                .connect_signaling(&server_addr, &session_id)
                .await
            {
            ...
            while let Some(msg) = send_queue_rx.recv().await {
                ...
                if let Some(ref debug_log) = debug_log_for_send {
                        ...
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] SendQueue: Processing message from queue",
                ...
                let result = match msg {
                    ...
                    QueuedMessage::RawInput {
                        ...
                        match transport
                            .channel(crate::transport::ChannelPurpose::Input)
                            .await
                        {
                            Ok(input_channel) => {
                            ...
                            Err(_) => {
                                ...
                                if data.len() > 1 && data[0] == 0x01 {
                                    ...
                                    let client_msg =
                                        crate::protocol::ClientMessage::TerminalInput {
                                            data: input_bytes.to_vec(),
                                            echo_local: None,
                                        };
                                    let app_msg =
                                        crate::protocol::signaling::AppMessage::Protocol {
                        ...
                        .open(debug_log)
                    {
                        ...
                        match result {
                            Ok(_) => {
                                let _ = writeln!(
                                    file,
                                    "[{}] SendQueue: Message sent successfully in {}ms",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    elapsed.as_millis()
                                );
                            }
                            Err(e) => {
                                let _ = writeln!(
                                    file,
                                    "[{}] SendQueue: Send failed after {}ms: {:?}",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    elapsed.as_millis(),
                                    e
                                );
    ...
    pub async fn start(self: Arc<Self>) -> Result<()> {
        ...
        if let Some(ref debug_log) = self.debug_log {
                ...
                .open(debug_log)
            {
                ...
                let _ = writeln!(
                    file,
                    "[{}] Client: About to enable raw mode and enter terminal UI",
                    chrono::Utc::now().format("%H:%M:%S%.3f")
                );
            }
        }
        ...
        // Enable mouse capture for scrollback functionality.
        // Text selection is available via Shift+drag (standard terminal behavior).
        ...
        if let Some(ref debug_log) = self.debug_log {
                ...
                .open(debug_log)
            {
                ...
                let _ = writeln!(
                    file,
                    "[{}] Client: Terminal UI initialized, starting event loops",
                    chrono::Utc::now().format("%H:%M:%S%.3f")
                );
            }
        }
        ...
        // Start event loops
        let result = self
            .run_event_loops(&mut terminal, &mut *server_rx, &mut *shutdown_rx)
            .await;
        ...
        if let Some(ref debug_log) = self.debug_log {
                ...
                .open(debug_log)
            {
                ...
                let _ = writeln!(
                    file,
                    "[{}] Client: Event loops exited with result: {:?}",
                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                    result
                );
            }
        ...
        *self.state.write().await = ClientState::Connecting;
        ...
        if let Some(ref debug_log) = self.debug_log {
                ...
                .open(debug_log)
            {
                ...
                let _ = writeln!(
                    file,
                    "[{}] Client: connect_and_subscribe() started",
                    chrono::Utc::now().format("%H:%M:%S%.3f")
                );
            }
        }
        ...
        /* strict WebRTC is now default */
        {
            ...
            if let Some(ref debug_log) = self.debug_log {
                    ...
                    .open(debug_log)
                {
                    ...
                    let _ = writeln!(
                        file,
                        "[{}] Client: Waiting for WebRTC initialization...",
                        chrono::Utc::now().format("%H:%M:%S%.3f")
                    );
                }
            }
            ...
            loop {
                ...
                let webrtc_ready = {
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client: WebRTC check - is_webrtc: {}, is_connected: {}",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                is_webrtc,
                                is_connected
                            );
                ...
                if webrtc_ready {
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client: WebRTC is ready, proceeding with subscription!",
                                chrono::Utc::now().format("%H:%M:%S%.3f")
                            );
                        }
                    }
                ...
                if start.elapsed() > timeout {
                    return Err(anyhow::anyhow!(
                        "Timed out waiting for WebRTC initialization"
                    ));
                }
        ...
        // Send subscription request with unified grid support
        let subscribe_msg = ClientMessage::Subscribe {
            subscription_id: self.subscription_id.clone(),
            dimensions,
            // New unified grid fields
            initial_fetch_size: Some(500), // Initial rows to load
            stream_history: Some(true),    // Enable history streaming
            // Legacy viewport-based fields (for backward compatibility)
        ...
        if let Some(ref debug_log) = self.debug_log {
                ...
                .open(debug_log)
            {
                ...
                let _ = writeln!(
                    file,
                    "[{}] Client: About to send Subscribe message",
                    chrono::Utc::now().format("%H:%M:%S%.3f")
                );
                let _ = writeln!(
                    file,
                    "[{}] Client sending Subscribe message: {:?}",
                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                    subscribe_msg
                );
                let _ = writeln!(
                    file,
                    "[{}] Client sending AppMessage: {:?}",
                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                    app_msg
                );
            }
        ...
        if let Some(ref debug_log) = self.debug_log {
                ...
                .open(debug_log)
            {
                ...
                let _ = writeln!(
                    file,
                    "[{}] Client: Subscribe message sent successfully!",
                    chrono::Utc::now().format("%H:%M:%S%.3f")
                );
            }
        }
        ...
        let ack_result =
            tokio::time::timeout(Duration::from_millis(sub_ack_timeout_ms), server_rx.recv()).await;
        ...
        match ack_result {
                ...
                *self.state.write().await = ClientState::Connected;
                ...
                if let Some(ref debug_log) = self.debug_log {
                        ...
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client received SubscriptionAck",
                            chrono::Utc::now().format("%H:%M:%S%.3f")
                        );
                    }
                }
            ...
            Ok(None) => {
                return Err(anyhow::anyhow!(
                    "Server connection closed while waiting for SubscriptionAck"
                ));
            }
            Err(_) => {
                return Err(anyhow::anyhow!(
        ...
        while !received_snapshot {
            ...
            let message_result =
                tokio::time::timeout(Duration::from_millis(remaining_timeout), server_rx.recv())
                    .await;
            ...
            match message_result {
                Ok(Some(ServerMessage::Snapshot { grid, sequence, .. })) => {
                    ...
                    *self.last_applied_seq.lock().await = sequence;
                    ...
                    if !renderer.unified_grid.is_empty() {
                        // Apply to unified grid
                        renderer.apply_initial_snapshot_unified(grid);
                    } else {
                        // Apply to legacy grid
                        renderer.apply_snapshot(grid);
                    }
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client received initial Snapshot (unified={})",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                !renderer.unified_grid.is_empty()
                            );
                        }
                    }
                }
                Ok(Some(ServerMessage::HistoryInfo {
                    oldest_line,
                    latest_line,
                    ..
                })) => {
                    ...
                    *self.latest_line.lock().await = Some(latest_line);
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client received HistoryInfo: oldest={}, latest={}",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                oldest_line,
                                latest_line
                            );
                ...
                Ok(Some(ServerMessage::HistoryMetadata {
                    total_lines,
                    oldest_line,
                    latest_line,
                    terminal_width,
                    ...
                    self.grid_renderer.lock().await.initialize_unified_grid(
                        total_lines,
                        oldest_line,
                        latest_line,
                        terminal_width,
                    ...
                    // Debug log unified grid initialization
                    if let Some(ref debug_log) = self.debug_log {
                        if let Ok(mut file) = std::fs::OpenOptions::new()
                            .create(true)
                            .append(true)
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client initialized unified grid: total={}, range={}..{}, size={}x{}",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                total_lines,
                                oldest_line,
                                latest_line,
                ...
                Ok(Some(msg)) => {
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client received unexpected message while waiting for Snapshot: {:?}",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                msg
                            );
                        }
                ...
                Ok(None) => {
                    return Err(anyhow::anyhow!(
                        "Server connection closed while waiting for Snapshot"
                    ));
                }
                Err(_) => {
                    return Err(anyhow::anyhow!(
    ...
    async fn run_event_loops<B: ratatui::backend::Backend>(
        ...
        if let Some(ref debug_log) = self.debug_log {
                ...
                .open(debug_log)
            {
                ...
                let _ = writeln!(
                    file,
                    "[{}] Client: Entered run_event_loops",
                    chrono::Utc::now().format("%H:%M:%S%.3f")
                );
            }
        }
            ...
            if loop_count == 0 {
                ...
                if let Some(ref debug_log) = self.debug_log {
                        ...
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: First iteration of event loop",
                            chrono::Utc::now().format("%H:%M:%S%.3f")
                        );
                    }
                }
                ...
                Some(evt) = event_rx.recv() => {
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                        if let Ok(mut file) = std::fs::OpenOptions::new().create(true).append(true).open(debug_log) {
                            ...
                            match &evt {
                                Event::Key(key) => {
                                    let _ = writeln!(file, "[{}] EventLoop: Key event={:?}",
                                        chrono::Utc::now().format("%H:%M:%S%.3f"), key);
                                }
                                Event::Mouse(mouse) => {
                                    let _ = writeln!(file, "[{}] EventLoop: Mouse event kind={:?}",
                                        chrono::Utc::now().format("%H:%M:%S%.3f"), mouse.kind);
                                }
                                _ => {
                                    let _ = writeln!(file, "[{}] EventLoop: Other event",
                                        chrono::Utc::now().format("%H:%M:%S%.3f"));
                                }
                            }
                        }
    ...
    async fn handle_mouse_event(&self, mouse: crossterm::event::MouseEvent) -> Result<()> {
        ...
        match mouse.kind {
            MouseEventKind::ScrollUp => {
                // Debug log: ScrollUp event processing
                if let Some(ref debug_log) = self.debug_log {
                    if let Ok(mut file) = std::fs::OpenOptions::new()
                        .create(true)
                        .append(true)
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] ScrollUp: About to call scroll_vertical(3)",
                            chrono::Utc::now().format("%H:%M:%S%.3f")
                        );
                    }
                }
                ...
                // Debug log: After scroll_vertical call
                if let Some(ref debug_log) = self.debug_log {
                    if let Ok(mut file) = std::fs::OpenOptions::new()
                        .create(true)
                        .append(true)
                        .open(debug_log)
                    {
                        ...
                        let metadata_info = if let Some(ref metadata) = renderer.history_metadata {
                            format!(
                                "metadata(oldest={}, latest={})",
                                metadata.oldest_line, metadata.latest_line
                            )
                        } else {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] ScrollUp: After scroll_vertical, offset={}, grid_end_line={:?}, {}, view_line={:?}",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            renderer.scroll_offset,
                            renderer.grid.end_line,
                            metadata_info,
                            renderer.historical_anchor()
                        );
                    }
                }
                ...
                if let Some(ref mut recorder) = *self.debug_recorder.lock().await {
                    ...
                    let _ = recorder.record_event(
                        crate::debug_recorder::DebugEvent::ClientScrollEvent {
            ...
            MouseEventKind::ScrollDown => {
                // Debug log: ScrollDown event processing
                if let Some(ref debug_log) = self.debug_log {
                    if let Ok(mut file) = std::fs::OpenOptions::new()
                        .create(true)
                        .append(true)
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] ScrollDown: About to call scroll_vertical(-3)",
                            chrono::Utc::now().format("%H:%M:%S%.3f")
                        );
                    }
                }
                ...
                if let Some(ref debug_log) = self.debug_log {
                        ...
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] ScrollDown: After scroll_vertical, offset={}, grid_end_line={:?}, {}, view_line={:?}",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            renderer.scroll_offset,
                            renderer.grid.end_line,
                            metadata_info,
    ...
    async fn handle_key_event(&self, key: KeyEvent) -> Result<()> {
        ...
        match key.code {
            KeyCode::Char(c) => {
                // Fixed predictive echo - don't modify the grid locally, just track predictions for rendering
                let bytes = vec![c as u8];

                // Get current cursor position for prediction tracking (but don't modify grid)
                let cursor_pos = {
                    let grid_renderer = self.grid_renderer.lock().await;
                    (grid_renderer.grid.cursor.col, grid_renderer.grid.cursor.row)
                };

                // Record prediction for rendering underlines (but don't apply to grid)
                {
                    let mut predictive_echo = self.predictive_echo.lock().await;
                    predictive_echo.predict_input(bytes.clone(), cursor_pos);
                }

                // Send to server with predictive echo enabled
                let client_msg = crate::protocol::ClientMessage::TerminalInput {
                    data: bytes,
                    echo_local: Some(true),
                };
            ...
            KeyCode::Enter => {
                let app_msg =
                    crate::protocol::signaling::AppMessage::TerminalInput { data: vec![b'\r'] };
                self.send_queue_tx
                    .send(QueuedMessage::AppMessage(app_msg))
                    .await?;
            }
            KeyCode::Tab => {
                let app_msg =
                    crate::protocol::signaling::AppMessage::TerminalInput { data: vec![b'\t'] };
                self.send_queue_tx
                    .send(QueuedMessage::AppMessage(app_msg))
                    .await?;
            }
            KeyCode::Backspace => {
                let app_msg =
                    crate::protocol::signaling::AppMessage::TerminalInput { data: vec![0x7F] };
                self.send_queue_tx
                    .send(QueuedMessage::AppMessage(app_msg))
                    .await?;
            }
                ...
                if scroll_offset > 0 {
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Exited scrollback mode with Escape",
                                chrono::Utc::now().format("%H:%M:%S%.3f")
                            );
                        }
                    }
            ...
            KeyCode::Down => {
                ...
                if let Some(ref debug_log) = self.debug_log {
                        ...
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Arrow Key DOWN: sending bytes {:?} (ESC[B)",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            bytes
                        );
                    }
            ...
            KeyCode::Home => {
                if key.modifiers.contains(KeyModifiers::SHIFT) {
                    ...
                    if let Some(metadata) = history_metadata {
                        ...
                        let max_scroll =
                            (latest_line - oldest_line + display_height as u64).saturating_sub(1);
                        {
                            let mut renderer = self.grid_renderer.lock().await;
                        ...
                        if let Some(ref debug_log) = self.debug_log {
                                ...
                                .open(debug_log)
                            {
                                ...
                                let _ = writeln!(
                                    file,
                                    "[{}] Shift+Home: scrolled to oldest line {}",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    oldest_line
                                );
                            }
                ...
                if key.modifiers.contains(KeyModifiers::SHIFT) {
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Shift+End: returned to realtime mode",
                                chrono::Utc::now().format("%H:%M:%S%.3f")
                            );
                        }
                    }
    ...
    async fn handle_server_message(&self, msg: ServerMessage) -> Result<()> {
                ...
                .open(debug_log)
            {
                ...
                let msg_type = match &msg {
                    ServerMessage::Delta {
                        ...
                        "Delta seq={} with {} changes",
                        sequence,
                        changes.cell_changes.len()
                    ),
                    ServerMessage::Snapshot { grid, sequence, .. } => {
                        format!("Snapshot seq={} {}x{}", sequence, grid.width, grid.height)
                    }
                    ServerMessage::SnapshotRange {
                        ...
                        "SnapshotRange seq={} watermark={} {}x{}",
                        sequence, watermark_seq, grid.width, grid.height
                    ),
                    ServerMessage::HistoryInfo {
                        ...
                        "HistoryInfo oldest={} latest={} total={}",
                        oldest_line, latest_line, total_lines
                    ),
                    ServerMessage::HistoryMetadata {
                        ...
                        "HistoryMetadata total={} range=({},{}) dims={}x{}",
                        total_lines, oldest_line, latest_line, terminal_width, terminal_height
                    ),
                    ServerMessage::HistoryChunk {
                        ...
                        "HistoryChunk lines=({},{}) rows={} final={}",
                        start_line,
                        end_line,
                        rows.len(),
                        is_final
                    ),
                    ServerMessage::ViewTransition {
                        from_mode, to_mode, ..
                    } => format!("ViewTransition {:?} -> {:?}", from_mode, to_mode),
                    ServerMessage::SubscriptionAck { status, .. } => {
                        format!("SubscriptionAck status={:?}", status)
                    }
                    ServerMessage::Error { code, message, .. } => {
                        format!("Error code={:?} msg={}", code, message)
                    }
                    ServerMessage::Pong { .. } => "Pong".to_string(),
                    ServerMessage::Notify { .. } => "Notify".to_string(),
                    ServerMessage::DeltaBatch { .. } => "DeltaBatch".to_string(),
                };
                let _ = writeln!(
                    file,
        ...
        match msg {
            ServerMessage::Delta {
                ...
                if let Some(ref path) = self.debug_log {
                        ...
                        .open(path)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: Received Delta seq={} changes={}",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            sequence,
                            changes.cell_changes.len()
                        );
                ...
                *self.last_applied_seq.lock().await = sequence;
                ...
                if let Some(ref mut recorder) = *self.debug_recorder.lock().await {
                    ...
                    let seam_end = seam_anchor
                        .saturating_add(6)
                        .min(renderer.grid.height.saturating_sub(1));
                    ...
                    for row in seam_start..=seam_end {
                        ...
                        for col in 0..renderer.grid.width.min(120) {
                            if let Some(cell) = renderer.grid.get_cell(row, col) {
                                line.push(cell.char);
                            }
                        }
                    ...
                    let blank_lines_before = renderer
                        .grid
                        .cells
                        .iter()
                    ...
                    let lines_before: Vec<String> = modified_lines
                        .iter()
                        .take(5)
                        .filter_map(|&line_num| {
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Cursor BEFORE applying delta seq={}: row={}, col={}",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                sequence,
                                before_cursor.row,
                                before_cursor.col
                            ...
                            if let Some(ref cursor_change) = changes.cursor_change {
                                let _ = writeln!(
                                    file,
                                    "[{}] Delta seq={} contains cursor change: old=({},{}) -> new=({},{})",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    sequence,
                                    cursor_change.old_position.row,
                                    cursor_change.old_position.col,
                                    cursor_change.new_position.row,
                                    cursor_change.new_position.col
                                );
                            }
                        }
                    }

                    // Apply the delta (supports both legacy and unified grid modes)
                    {
                        let mut renderer = self.grid_renderer.lock().await;
                        if !renderer.unified_grid.is_empty() {
                            renderer.apply_delta_unified(&changes);
                        } else {
                            renderer.apply_delta(&changes);
                        }
                    }
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Cursor AFTER applying delta seq={}: row={}, col={}",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                sequence,
                                renderer.grid.cursor.row,
                                renderer.grid.cursor.col
                            );

                            // Log if cursor position actually changed
                            if before_cursor.row != renderer.grid.cursor.row
                                || before_cursor.col != renderer.grid.cursor.col
                            {
                                let _ = writeln!(
                                    file,
                                    "[{}] Cursor position CHANGED during delta seq={}: ({},{}) -> ({},{})",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    sequence,
                                    before_cursor.row,
                                    before_cursor.col,
                    ...
                    let blank_lines_after = renderer
                        .grid
                        .cells
                        .iter()
                        .filter(|row| row.is_empty() || row.iter().all(|cell| cell.char == ' '))
                    ...
                    let lines_after: Vec<String> = modified_lines
                        .iter()
                        .take(5)
                        .filter_map(|&line_num| {
                            renderer.grid.cells.get(line_num as usize).map(|row| {
                    ...
                    let _ = recorder.record_event(
                        crate::debug_recorder::DebugEvent::ClientDeltaApplication {
                            timestamp: chrono::Utc::now(),
                            sequence,
                    ...
                    let _ = recorder.record_client_seam_context(
                        sequence,
                        seam_start,
                        &seam_before_lines,
                        &seam_after_lines,
                    ...
                    let _ = recorder.record_grid_bottom_context(
                        "client_after_delta",
                        &renderer.grid,
                        6,
                    );
                    ...
                    if blank_lines_before != blank_lines_after {
                        if let Some(ref path) = self.debug_log {
                                ...
                                std::fs::OpenOptions::new().append(true).open(path)
                            {
                                ...
                                let _ = writeln!(
                                    file,
                                    "[{}] Client: Delta {} changed blank lines: {} -> {} (lines modified: {:?})",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    sequence,
                                    blank_lines_before,
                                    blank_lines_after,
                    ...
                    if std::env::var("BEACH_SEAM_DEBUG").ok().is_some() {
                        if let Some(ref path) = self.debug_log {
                                ...
                                std::fs::OpenOptions::new().append(true).open(path)
                            {
                                ...
                                let _ = writeln!(
                                    file,
                                    "[{}] Client: BottomContext after delta dims {}x{} trailing_blanks {}",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    renderer.grid.width,
                    ...
                    // Clear predictions when server delta arrives (means input was processed)
                    {
                        let mut predictive_echo = self.predictive_echo.lock().await;
                        predictive_echo.clear();
                    }
                    ...
                    if let Ok(mut file) = std::fs::OpenOptions::new().append(true).open(debug_log) {
                        ...
                        if sequence != last_applied + 1 {
                            if sequence > last_applied + 1 {
                                let _ = writeln!(
                                    file,
                                    "[{}] SEQUENCE GAP DETECTED: Expected seq={}, got seq={} (gap of {})",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    last_applied + 1,
                                    sequence,
                                    sequence - last_applied - 1
                                );
                            } else if sequence <= last_applied {
                                let _ = writeln!(
                                    file,
                                    "[{}] OUT-OF-ORDER DELTA: seq={} <= last_applied={} (duplicate or redelivery)",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    sequence,
                                    last_applied
                                );
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Applied delta seq={} (last_applied={})",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            sequence,
                            last_applied
                        );
            ...
            ServerMessage::Snapshot { grid, sequence, .. } => {
                if let Some(ref path) = self.debug_log {
                        ...
                        .open(path)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: Received Snapshot seq={} dims={}x{} start={:?} end={:?}",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            sequence,
                            grid.width,
                            grid.height,
                ...
                *self.last_applied_seq.lock().await = sequence;
                ...
                // Apply the snapshot (supports both legacy and unified grid modes)
                {
                    let mut renderer = self.grid_renderer.lock().await;
                    // Check if in unified grid mode
                    if !renderer.unified_grid.is_empty() {
                        renderer.apply_initial_snapshot_unified(grid.clone());
                    } else {
                        renderer.apply_snapshot(grid.clone());
                    }
                }
                ...
                if let Some(ref path) = self.debug_log {
                        ...
                        .open(path)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: Snapshot applied (viewport_mode, start_line={}, end_line={}, dims={}x{})",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            start_u64,
                            end_u64,
                            renderer.grid.width,
                        ...
                        for row in 0..renderer.grid.height.min(5) {
                            ...
                            for col in 0..renderer.grid.width.min(80) {
                                if let Some(c) = renderer.grid.get_cell(row, col) {
                                    s.push(c.char);
                                }
                            }
                            let _ = writeln!(
                    ...
                    if std::env::var("BEACH_SEAM_DEBUG").ok().is_some() {
                        if let Some(ref path) = self.debug_log {
                                ...
                                std::fs::OpenOptions::new().append(true).open(path)
                            {
                                ...
                                let _ = writeln!(
                                    file,
                                    "[{}] Client: BottomContext after snapshot dims {}x{} trailing_blanks {}",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    renderer.grid.width,
                                    renderer.grid.height,
                                    trailing_blanks
            ...
            ServerMessage::SnapshotRange { grid, sequence, .. } => {
                if let Some(ref path) = self.debug_log {
                        ...
                        .open(path)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: Received SnapshotRange seq={} dims={}x{} start={:?} end={:?}",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            sequence,
                            grid.width,
                            grid.height,
                ...
                *self.last_applied_seq.lock().await = sequence;
                ...
                // SnapshotRange includes prefetch; anchoring at grid.start_line can pin to 0.
                {
                    ...
                    let anchor = {
                        ...
                        let vp_debug = current_vp
                            .as_ref()
                            .map(|vp| format!("({}, {})", vp.start_line, vp.end_line))
                        ...
                        let calculated_anchor = if let Some(vp) = current_vp {
                            ...
                            vp.start_line
                                .max(start)
                                .min(end.saturating_sub(viewport_h.saturating_sub(1)))
                        } else {
                        ...
                        if let Some(ref path) = self.debug_log {
                                ...
                                .open(path)
                            {
                                ...
                                let _ = writeln!(
                                    file,
                                    "[{}] Client: ANCHOR_CALC current_vp={} grid_range=({}, {}) calculated_anchor={}",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    vp_debug,
                                    start,
                                    end,
                    ...
                    if let Some(ref mut recorder) = *self.debug_recorder.lock().await {
                        let _ = recorder.record_client_grid_state(
                            &gr.grid,
                            gr.scroll_offset as i64,
                            "historical",
                        );
                        let _ = recorder.record_grid_bottom_context(
                            "client_after_snapshot_range",
                            &gr.grid,
                            6,
                        );
                    }
                    ...
                    if let Some(ref path) = self.debug_log {
                            ...
                            .open(path)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client: SnapshotRange anchor set to {} (viewport_h={}) start={} end={}",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                anchor,
                                viewport_h,
                                start,
                            ...
                            for row in preview_rows {
                                ...
                                for col in 0..gr.grid.width.min(100) {
                                    if let Some(cell) = gr.grid.get_cell(row, col) {
                                        line.push(cell.char);
                                    }
                                }
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client: SnapshotRange TOP non_blank_count={} start_line={:?} end_line={:?}",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                non_blank,
                                gr.grid.start_line.to_u64(),
                                gr.grid.end_line.to_u64()
                            );
                        }
                    }
                }
            ...
            // NEW: Handle unified grid messages
            ServerMessage::HistoryMetadata {
                total_lines,
                oldest_line,
                latest_line,
                ...
                if let Some(ref path) = self.debug_log {
                        ...
                        .open(path)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: Received HistoryMetadata total={} range=({},{}) dims={}x{} - switching to unified grid mode",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            total_lines,
                            oldest_line,
                            latest_line,
                ...
                // Switch to unified grid mode
                let debug_size = self.grid_renderer.lock().await.debug_size;
                match crate::client::grid_renderer::GridRenderer::new_with_unified_metadata(
                    total_lines,
                    oldest_line,
                    latest_line,
                    terminal_width,
                    ...
                    Ok(new_renderer) => {
                        *self.grid_renderer.lock().await = new_renderer;
                        if let Some(ref path) = self.debug_log {
                            if let Ok(mut file) = std::fs::OpenOptions::new()
                                .create(true)
                                .append(true)
                                .open(path)
                            {
                                ...
                                let _ = writeln!(
                                    file,
                                    "[{}] Client: Successfully switched to unified grid with {} total lines",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    total_lines
                                );
                            }
                    ...
                    Err(e) => {
                        if let Some(ref path) = self.debug_log {
                                ...
                                .open(path)
                            {
                                ...
                                let _ = writeln!(
                                    file,
                                    "[{}] Client: Failed to create unified grid renderer: {:?}",
                                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                                    e
                                );
                            }
            ...
            ServerMessage::HistoryChunk {
                ...
                if let Some(ref path) = self.debug_log {
                        ...
                        .open(path)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: Received HistoryChunk lines=({},{}) rows={} final={}",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            start_line,
                            end_line,
                            rows.len(),
                ...
                // Apply history chunk to unified grid
                self.grid_renderer
                    .lock()
                    .await
                    .apply_history_chunk_unified(start_line, end_line, rows);
                ...
                if is_final {
                    if let Some(ref path) = self.debug_log {
                            ...
                            .open(path)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client: Completed loading all history chunks",
                                chrono::Utc::now().format("%H:%M:%S%.3f")
                            );
                        }
                    }
            ...
            ControlMessage::Input { bytes, .. } => {
                ...
                if let Some(ref debug_log) = self.debug_log {
                        ...
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: Queueing TerminalInput: {} bytes (will try raw channel)",
                            chrono::Utc::now().format("%H:%M:%S%.3f"),
                            bytes.len()
                        );
                    }
                ...
                let has_input_channel = {
                    ...
                    transport
                        .channels()
                        .contains(&crate::transport::ChannelPurpose::Input)
                };
                ...
                if has_input_channel {
                    ...
                    if let Some(ref debug_log) = self.debug_log {
                            ...
                            .open(debug_log)
                        {
                            ...
                            let _ = writeln!(
                                file,
                                "[{}] Client: Queued JSON input in {}μs (no Input channel)",
                                chrono::Utc::now().format("%H:%M:%S%.3f"),
                                queue_start.elapsed().as_micros()
                            );
                        }
            ...
            *help_shown = true;
            ...
            if let Some(ref debug_log) = self.debug_log {
                    ...
                    .open(debug_log)
                {
                    ...
                    let _ = writeln!(
                        file,
                        "[{}] HELP: Scrollback controls - Shift+PageUp/Down: scroll | Shift+Home: top | Shift+End: bottom | ESC: exit | Mouse wheel: scroll",
                        chrono::Utc::now().format("%H:%M:%S%.3f")
                    );
                }
            }
    ...
    async fn handle_scroll_update(&self) -> Result<()> {
        // Check if we're in unified grid mode - if so, skip ViewportChanged logic
        {
            let gr = self.grid_renderer.lock().await;
            if !gr.unified_grid.is_empty() {
                // In unified grid mode, all scrolling is handled client-side
                // No need to send ViewportChanged messages to server
                if let Some(ref debug_log) = self.debug_log {
                    if let Ok(mut file) = std::fs::OpenOptions::new()
                        .create(true)
                        .append(true)
                        .open(debug_log)
                    {
                        ...
                        let _ = writeln!(
                            file,
                            "[{}] Client: Skipping ViewportChanged - using unified grid mode",
                            chrono::Utc::now().format("%H:%M:%S%.3f")
                        );
                    }
                }
        ...
        // Capture scroll state and mode (legacy viewport mode)
        ...
        let new_viewport = crate::protocol::subscription::messages::Viewport {
            start_line: viewport_start,
            end_line: viewport_end,
        };
        ...
        if let Some(ref debug_log) = self.debug_log {
            ...
            {
                use std::io::Write;
                let mode = if view_line_opt.is_some() {
                    "historical"
                } else {
                    "realtime"
                };
                let _ = writeln!(
                    file,
                    "[{}] Client: VIEWPORT_CALC mode={} inputs(latest_line={}, view_line={:?}, scroll_offset={}, viewport_height={}) -> viewport=({}, {})",
                    chrono::Utc::now().format("%H:%M:%S%.3f"),
                    mode,
                    latest_line,
                    view_line_opt,
        ...
        if viewport_changed {
            ...
            if let Some(ref debug_log) = self.debug_log {
                    ...
                    .open(debug_log)
                {
                    ...
                    let _ = writeln!(
                        file,
                        "[{}] Client: ViewportChanged computed mode={} scroll_offset={} latest_line={} viewport=({}, {}) follow_tail={}",
                        chrono::Utc::now().format("%H:%M:%S%.3f"),
                        mode,
                        scroll_offset,
                        latest_line,
            ...
            if let Some(ref mut recorder) = *self.debug_recorder.lock().await {
                let _ = recorder.record_event(
                    crate::debug_recorder::DebugEvent::ClientHistoryNeedsCheck {
                        timestamp: chrono::Utc::now(),
            ...
            let app_msg = AppMessage::Protocol {
                message: serde_json::to_value(&viewport_msg)?,
            };
            self.send_queue_tx
                .send(QueuedMessage::AppMessage(app_msg))
                .await
    ...
    /// Render the terminal
    async fn render<B: ratatui::backend::Backend>(&self, terminal: &mut Terminal<B>) -> Result<()> {
        let grid_renderer = self.grid_renderer.lock().await;
        let predictive_echo = self.predictive_echo.lock().await;

        // Pass a non-empty predictions array if we have any pending predictions
        // The actual positions don't matter - renderer will check cursor position
        let predictions: Vec<(u16, u16)> = if predictive_echo.active_predictions().is_empty() {
            Vec::new()
        } else {
            vec![(0, 0)] // Dummy position - renderer uses cursor position instead
        };

        terminal.draw(|f| {
            // Use unified grid render if in unified mode, otherwise use legacy render
            if !grid_renderer.unified_grid.is_empty() {
                grid_renderer.render_unified(f);
            } else {
