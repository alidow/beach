...
- Controller/state/ack traffic rides the unified Beach transport only—no dedicated `mgr-actions`/`mgr-acks`/`mgr-state` channels or HTTP polling. If you see logic that opens extra data channels or falls back to HTTP for controller/state, delete it.
...
- Cache/state syncing between host ↔ manager or host ↔ browser must never use HTTP. Unified WebRTC + wss signaling only—no host-side HTTP pollers or publishers.
