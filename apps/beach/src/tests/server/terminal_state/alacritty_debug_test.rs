...
fn test_debug_alacritty_grid_direct() {
    ...
    use alacritty_terminal::vte::ansi::{Handler, CharsetIndex, StandardCharset};
    ...
    println!("Grid dimensions: {} lines x {} columns", term.grid().screen_lines(), term.grid().columns());
    println!("Cursor: line={}, col={}", term.grid().cursor.point.line.0, term.grid().cursor.point.column.0);
