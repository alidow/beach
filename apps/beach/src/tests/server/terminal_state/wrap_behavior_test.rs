...
fn test_prompt_eol_exact_sequence() {
    ...
    println!("Cursor: row={}, col={}", initial_grid.cursor.row, initial_grid.cursor.col);
    ...
    println!("Cursor: row={}, col={}", after_hello.cursor.row, after_hello.cursor.col);
    
    // Step 2: PROMPT_EOL_MARK sequence starts
    // 2a. Bold + Reverse video for %
    term.process_output(b"\x1b[1m\x1b[7m%\x1b[27m\x1b[1m\x1b[0m").unwrap();
    let after_percent = term.get_current_grid();
    println!("\n=== After % with styling ===");
    println!("Cursor: row={}, col={}", after_percent.cursor.row, after_percent.cursor.col);
    ...
    println!("Cursor: row={}, col={}", after_spaces.cursor.row, after_spaces.cursor.col);
    ...
    println!("\n=== After first CR ===");
    println!("Cursor: row={}, col={}", after_cr1.cursor.row, after_cr1.cursor.col);
    ...
    println!("Cursor: row={}, col={}", after_space.cursor.row, after_space.cursor.col);
    
    // 2e. Another CR
    term.process_output(b"\r").unwrap();
    let after_cr2 = term.get_current_grid();
    println!("\n=== After second CR ===");
    println!("Cursor: row={}, col={}", after_cr2.cursor.row, after_cr2.cursor.col);
    
    // 2f. Another CR (yes, zsh does three CRs)
    term.process_output(b"\r").unwrap();
    let after_cr3 = term.get_current_grid();
    println!("\n=== After third CR ===");
    println!("Cursor: row={}, col={}", after_cr3.cursor.row, after_cr3.cursor.col);
    
    // 2g. Reset styles and clear to end of screen
    term.process_output(b"\x1b[0m\x1b[27m\x1b[24m\x1b[J").unwrap();
    let after_clear = term.get_current_grid();
    println!("\n=== After style reset and ESC[J ===");
    println!("Cursor: row={}, col={}", after_clear.cursor.row, after_clear.cursor.col);
    
    // Step 3: Output the prompt
    term.process_output(b"(base) prompt> ").unwrap();
    let final_grid = term.get_current_grid();
    println!("\n=== After prompt ===");
    println!("Cursor: row={}, col={}", final_grid.cursor.row, final_grid.cursor.col);
...
fn test_wrap_with_one_extra_char() {
    ...
    assert_eq!(grid.get_cell(1, 0).unwrap().char, 'X', "X should wrap to next line");
...
fn test_wrap_then_cr_behavior() {
    ...
    println!("\nBefore CR:");
    println!("Cursor: row={}, col={}", before_cr.cursor.row, before_cr.cursor.col);
    ...
    println!("After CR:");
    println!("Cursor: row={}, col={}", after_cr.cursor.row, after_cr.cursor.col);
