...
fn test_basic_debug_view() {
    ...
    assert!(full_text.contains("$ echo 'hello'"), "Should contain command");
...
fn test_debug_view_with_height_limit() {
    ...
    // Add some content
    {
        let mut backend = backend_arc.lock().unwrap();
        for i in 1..=10 {
            backend.process_output(format!("Line {}\r\n", i).as_bytes()).unwrap();
        }
    }
...
fn test_debug_view_time_travel() {
    ...
    let past = debug_handler.get_grid_view(None, Some(time_middle), None).unwrap();
    let past_text = past.rows.join("\n");
    assert!(past_text.contains("Initial"));
    assert!(!past_text.contains("Later"), "Past view should not contain later output");
    
    // Verify timestamp is in the past
    assert!(past.timestamp <= time_middle, "Timestamp should be from requested time or earlier");
}
...
fn test_debug_view_from_non_existent_line() {
    ...
    match &result {
        Ok(response) => {
            println!("Got response for line 1000: start_line={}, end_line={} (returns last view)", 
    ...
    assert!(result.is_ok(), "Should return last view when requesting beyond max line");
}
...
fn test_debug_view_boundary_conditions() {
    ...
    println!("Terminal has lines {} to {}", full_view.start_line, last_line);
    ...
    assert!(result.is_ok(), "Should return last view when requesting beyond max");
}
...
fn test_debug_view_with_ansi_colors() {
    ...
    {
        let mut backend = backend_arc.lock().unwrap();
        backend.process_output(b"\x1b[31mRed Text\x1b[0m\r\n").unwrap();
    }
    ...
    assert!(response.ansi_rows.is_some(), "Should include ANSI colored rows");
    
    let ansi_rows = response.ansi_rows.unwrap();
    let ansi_text = ansi_rows.join("\n");
    
    // Should contain ANSI escape codes for red color
    assert!(ansi_text.contains("\x1b[") || ansi_text.contains("Red Text"), 
...
fn test_line_tracking_across_scrolling() {
    ...
    assert!(response.end_line >= response.start_line, "End line should be >= start line");
    
    // The visible content should be the later lines (due to scrolling)
    let full_text = response.rows.join("\n");
    assert!(full_text.contains("Line 15") || full_text.contains("Line 14"), 
