...
- **CLI & web toggles:** terminal fallback requests now honour CLI flags (`--fallback-cohort`, `--fallback-entitlement-proof`, `--fallback-telemetry-opt-in`) alongside Beach Auth profiles, and beach-web exposes matching advanced inputs plus a Beach Auth call-to-action when fallback is locked — giving rollout teams a UI/CLI surface while full browser auth lands; paused responses surface a friendly error to users.
