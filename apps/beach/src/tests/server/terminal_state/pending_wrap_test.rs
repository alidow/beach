...
fn test_pending_wrap_vs_immediate_wrap() {
    ...
    println!(
        "Cursor position: row={}, col={}",
        grid_with_prompt.cursor.row, grid_with_prompt.cursor.col
    );
...
fn test_wrap_at_exact_boundary() {
    ...
    assert_eq!(
        ...
        "CR should keep us on same row in pending wrap state"
    );
    assert_eq!(
        grid_after_cr.cursor.col, 0,
        "CR should move cursor to column 0"
    );
