...
fn test_background_color_preserved_on_clear() {
        ...
        for col in 0..5 {
            if let Some(cell) = grid.get_cell(row, col) {
                assert_eq!(
                    ...
                    "Cell at ({}, {}) should have red background",
                    row, col
                );
                assert_eq!(
                    cell.char, ' ',
                    "Cell at ({}, {}) should be a space after clear",
                    row, col
                );
            }
        }
    ...
    assert_eq!(
        grid.cursor.row, 0,
        "Cursor should be at row 0 after \\x1b[H"
    );
    assert_eq!(
        grid.cursor.col, 0,
        "Cursor should be at col 0 after \\x1b[H"
    );
}
...
fn test_erase_line_preserves_background() {
    ...
    for col in 0..10 {
        if let Some(cell) = grid.get_cell(0, col) {
            assert_eq!(
                cell.bg_color, expected_bg,
                "Cell at (0, {}) should have green background after erase line",
                col
            );
            assert_eq!(
                cell.char, ' ',
                "Cell at (0, {}) should be a space after erase line",
                col
            );
        }
    }
...
fn test_partial_clear_preserves_background() {
    ...
    for col in 39..45 {
        if let Some(cell) = grid.get_cell(11, col) {
            assert_eq!(
                cell.bg_color, expected_bg,
                "Cell at (11, {}) should have blue background",
                col
            );
        }
    }
    ...
    for col in 0..3 {
        if let Some(cell) = grid.get_cell(0, col) {
            assert_eq!(
                cell.bg_color,
                Color::Indexed(1),
                "First part should have red background at col {}",
                col
            );
        }
    }
    ...
    for col in 8..10 {
        if let Some(cell) = grid.get_cell(0, col) {
            assert_eq!(
                cell.bg_color,
                Color::Default,
                "Text after SGR reset should have default background at col {}",
                col
            );
        }
    }
