...
fn test_terminal_resize_preserves_content() {
    ...
    assert!(
        found_testing,
        "Content 'Testing resize' was lost during resize"
    );
}
...
fn test_resize_creates_dimension_change_delta() {
    ...
    let has_dimension_change = history_lock
        .iter_deltas()
        .any(|delta| delta.dimension_change.is_some());

    assert!(
        has_dimension_change,
        "No dimension change recorded in deltas"
    );

    // Check the dimension change details
    let dimension_delta = history_lock
        .iter_deltas()
...
fn test_multiple_resizes_tracked_correctly() {
    ...
    let dimension_changes: Vec<_> = history_lock
        .iter_deltas()
...
fn test_resize_timing_in_output_sequence() {
    ...
    let resize_position = history_lock
        .iter_deltas()
