...
- Telemetry expectations: host-applied versions publish a single `canvas.measurement` event, while any DOM payload suppressed by host precedence emits `canvas.measurement.dom-skipped-after-host` (tagged with the stage that rejected it) and advancing DOM measurements pick up `canvas.measurement.dom-advanced-after-host` so ops can spot multi-transport oscillation without double-counting measurements.
