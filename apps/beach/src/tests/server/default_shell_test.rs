...
async fn test_default_shell_basic_echo() {
    ...
    assert!(
        ...
        "Output should contain 'world'. Got: {:?}",
        lines
    );
}
...
async fn test_default_shell_line_wrapping() {
    ...
    let output_line = lines.iter().find(|line| {
    ...
    assert!(
        output_line.is_some(),
        "Should find output line with 'a's. Lines: {:?}",
        lines
    );
...
async fn test_pty_handles_special_characters() {
    ...
    assert!(
        output.contains("tab") && output.contains("here"),
...
async fn test_terminal_resize_with_complex_content() {
    ...
    let a_lines_80: Vec<&str> = output_80_cols
        .lines()
    ...
    assert!(
        a_lines_80.len() >= 2,
        "100 A's should wrap in 80-column terminal"
    );
    ...
    let a_lines_40: Vec<&str> = output_40_cols
        .lines()
        .filter(|line| line.contains("AAA"))
        .collect();
    ...
    let a_lines_120: Vec<&str> = output_120_cols
        .lines()
        .filter(|line| line.contains("AAA"))
        .collect();
    ...
    assert!(
        a_lines_120.len() <= a_lines_80.len(),
    ...
    for width in [60, 45, 90, 30] {
        ...
        assert!(
            output.contains("resize test"),
            "Terminal should still work after resizing to {} cols",
            width
        );
    }
}
...
async fn test_vim_tui_interaction() {
    ...
    session
        .send_command(&format!("echo 'initial content' > {}", test_file))
        .await;
    sleep(Duration::from_millis(200)).await;
    session.clear_output();
    ...
    assert!(
        content.contains("Hello from test") || content.contains("initial content"),
...
async fn test_vim_with_terminal_resize() {
    ...
    let wide_content = (1..=30)
        .map(|i| format!("Line {}: {}", i, "=".repeat(100)))
        .collect::<Vec<_>>()
        .join("\n");
    session
        .send_command(&format!(
            "cat > {} << 'EOF'\n{}\nEOF",
            test_file, wide_content
        ))
        .await;
    sleep(Duration::from_millis(300)).await;
    session.clear_output();
...
async fn test_special_key_sequences() {
    ...
    assert!(
        ...
        "Ctrl+C should interrupt sleep command"
    );
    ...
    assert!(
        cat_output.contains("test input"),
        "Cat should echo the input"
    );
    ...
    assert!(
        edited_output.contains("hello") || edited_output.contains("INSERTED"),
