...
- [x] M3b: Connectivity smoke ready: bring up the dedicated smoke stack and pass the 60s WebRTC/cache sync smoke (host ↔ manager-rewrite) on alternate ports. _Smoke stack now disables NAT hinting in-Docker and uses the shared bridge (`BEACH_DISABLE_NAT_HINT=1`, ICE servers empty host-only). RTC bus integration test runs inside the manager container against the in-stack host and now passes end-to-end (builds cached target volume and forces a clean exit after assertions to avoid lingering WebRTC tasks). Cache sync path remains green._
...
- [x] M3d: Host bus adoption (partial): remove controller auto-attach gating in `apps/beach`, always stand up the bus on unified transport, and drop legacy fastpath peers. _Done: host now instantiates the unified bus unconditionally; fastpath/HTTP controller plumbing removed earlier; explicit manager topic constants/publisher/subscriber helpers added for auditability._
- [x] M3e: Buggy controller over bus: beach-buggy consumes `controller/input` and publishes `controller/ack|state|health` on the bus; host no longer handles controller logic directly. _Buggy already exposes `publisher.rs`/`subscriber.rs` on `beach.manager.*`; host unified bridge uses shared topics and delegates controller input/acks purely via bus._
...
- [x] M3g: Subscriber/publisher clarity: each participant (host/beach-buggy/manager) has explicit `subscriber.rs`/`publisher.rs` modules listing topics and handling, to make the bus wiring easy to audit. _Manager rewrite now exposes `bus::{subscriber,publisher}` re-exports with topic constants; host bus now has explicit manager topics + publisher/subscriber helpers (and unified bridge consumes the shared topic constants). Beach-buggy already had subscriber/publisher modules._
...
- [ ] M4a: Extension→queue wiring: action/ack/state extensions flow into Redis Streams with backpressure limits; tests cover drop/queue/ack paths in-memory + Redis. _In progress: manager rewrite in-memory queue now enforces per-lane caps with drop counters/tests; Redis queue trims lists to a 10k cap and enqueued metrics fire._
- [x] M4a.1: In-memory queue adapter for action/ack/state with backpressure counters + unit tests. _Added `InMemoryQueue` drop counters in manager rewrite + drop test._
- [x] M4a.2: Redis Streams adapter (config, connection manager, consumer group setup) plus integration test gated behind `redis-queue` feature. _Implemented `RedisQueue` (manager rewrite) with basic LTRIM backpressure; ignored round-trip test gated on `REDIS_URL`._
...
- [ ] M4b: Persistence: controller leases/events persisted to Postgres with RLS; idempotent writes and retries; Postgres-backed tests. _Metrics wired for persist success/error; Postgres adapter (SeaORM) selected when `DATABASE_URL` is set; falling back to Redis/memory with a log if connect fails. Ignored Redis roundtrip test remains; ignored PG round-trip added (`tests/persistence_pg.rs`) gated on `DATABASE_URL`/`PG_URL`._
  ...
  - [x] M5.5: Early smoke: stand up two rewrite managers in smoke stack, hash a host to one, kill it, and assert reassignment occurs. _Added `apps/beach-manager-rewrite/tests/smoke/docker-compose.failover.yml` + `run-failover-smoke.sh` to spin two managers sharing Redis/Road. Script attaches via manager-1, stops it, waits TTL, and asserts manager-2 assigns the host._
...
**Next up:** Start M6 (API surface) and then M9b controller-path smoke once controller bus is fully stable.
...
- Incremental rollout plan:
  ...
  - Note: for smokes, set a known passphrase via `BEACH_HOST_PASSPHRASE` and pass it through to manager via `BEACH_WEBRTC_ATTACH_PASSPHRASE` so RTC attach is authorized; keep signaling logs on for attach failures.
