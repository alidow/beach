...
impl MediaEncryptor {
    fn new(key: &[u8; 32], handshake_hash: &[u8]) -> Self {
        Self { cipher: ChaCha20Poly1305::new(Key::from_slice(key)), counter: 0, aad: handshake_hash.to_vec() }
    }

    pub fn seal(&mut self, plaintext: &[u8]) -> Result<MediaFrame, NoiseError> {
        let nonce = next_nonce(&mut self.counter);
        let ciphertext = self.cipher.encrypt(Nonce::from_slice(&nonce), Payload { msg: plaintext, aad: &self.aad }).map_err(|err| NoiseError::Cipher(err.to_string()))?;
        Ok(MediaFrame { version: MEDIA_FRAME_VERSION, nonce, ciphertext })
    }
}
...
impl MediaDecryptor {
    fn new(key: &[u8; 32], handshake_hash: &[u8]) -> Self {
        Self { cipher: ChaCha20Poly1305::new(Key::from_slice(key)), last_counter: None, aad: handshake_hash.to_vec() }
    }

    pub fn open(&mut self, frame: &MediaFrame) -> Result<Vec<u8>, NoiseError> {
        if frame.version != MEDIA_FRAME_VERSION { return Err(NoiseError::UnsupportedFrame(frame.version)); }
        let counter = counter_from_nonce(&frame.nonce);
        if let Some(last) = self.last_counter { if counter <= last { return Err(NoiseError::Replay(counter)); } }
        let plaintext = self.cipher.decrypt(Nonce::from_slice(&frame.nonce), Payload { msg: &frame.ciphertext, aad: &self.aad }).map_err(|err| NoiseError::Cipher(err.to_string()))?;
        self.last_counter = Some(counter);
        Ok(plaintext)
    }
}

fn derive_transport_keys(psk: &[u8; 32], handshake_hash: &[u8], local_id: &str, remote_id: &str) -> Result<TransportKeys, NoiseError> {
    ...
    hkdf.expand(send_label.as_bytes(), &mut send_key).map_err(|_| NoiseError::Cipher("hkdf expand failure".into()))?;
    hkdf.expand(recv_label.as_bytes(), &mut recv_key).map_err(|_| NoiseError::Cipher("hkdf expand failure".into()))?;
    ...
    hkdf.expand(verify_label.as_bytes(), &mut verify_bytes).map_err(|_| NoiseError::Cipher("hkdf expand failure".into()))?;
    let code = u32::from_le_bytes(verify_bytes) % 1_000_000;
    let verification_code = format!("{code:06}");
    Ok(TransportKeys { send_key, recv_key, verification_code })
}

fn build_prologue(handshake_id: &HandshakeId, local_id: &str, remote_id: &str, context: &[u8]) -> Vec<u8> {
    let mut peers = [local_id.as_bytes(), remote_id.as_bytes()];
    peers.sort();
    let mut prologue = Vec::with_capacity(PROLOGUE_PREFIX.len() + handshake_id.to_base64().len() + peers[0].len() + peers[1].len() + context.len() + 5);
...
fn next_nonce(counter: &mut u64) -> [u8; 12] { let value = *counter; *counter = counter.wrapping_add(1); counter_to_nonce(value) }
fn counter_from_nonce(nonce: &[u8; 12]) -> u64 { let mut bytes = [0u8; 8]; bytes.copy_from_slice(&nonce[4..]); u64::from_be_bytes(bytes) }
fn counter_to_nonce(counter: u64) -> [u8; 12] { let mut nonce = [0u8; 12]; nonce[4..].copy_from_slice(&counter.to_be_bytes()); nonce }
...
impl NoiseController {
    pub fn new(config: HandshakeConfig<'_>) -> Result<Self, NoiseError> {
        ...
        Ok(Self { handshake: Some(handshake), pending_outgoing, encryptor: None, decryptor: None, transport_keys: None, handshake_hash: None, verification_code: None })
    }

    fn finish_handshake(&mut self, handshake: NoiseHandshake) -> Result<(), NoiseError> {
        let session = handshake.finalize()?;
        let NoiseSession { handshake_hash, keys } = session;
    ...
    pub fn take_outgoing(&mut self) -> Option<Vec<u8>> { self.pending_outgoing.pop_front() }
    ...
    pub fn process_incoming(&mut self, message: &[u8]) -> Result<Option<Vec<u8>>, NoiseError> {
        if self.handshake.is_some() {
            ...
            let decryptor = self.decryptor.as_mut().ok_or_else(|| NoiseError::Incomplete)?;
            let plaintext = decryptor.open(&frame)?;
            Ok(Some(plaintext))
        }
    }

    pub fn handshake_complete(&self) -> bool { self.encryptor.is_some() && self.decryptor.is_some() }
    pub fn verification_code(&self) -> Option<&str> { self.verification_code.as_deref() }
    pub fn handshake_hash(&self) -> Option<&[u8]> { self.handshake_hash.as_deref() }

    pub fn seal_media(&mut self, plaintext: &[u8]) -> Result<Vec<u8>, NoiseError> {
        let enc = self.encryptor.as_mut().ok_or_else(|| NoiseError::Incomplete)?;
        let frame = enc.seal(plaintext)?;
        Ok(frame.encode())
    }
}
...
mod tests {
    ...
    fn material() -> SessionMaterial { SessionMaterial::derive("session-abc", "passcode123").expect("material") }
    fn handshake_id() -> HandshakeId { HandshakeId::from_base64("AAAAAAAAAAAAAAAAAAAAAA==").expect("handshake parse") }

    #[test]
    fn handshake_produces_symmetric_keys_and_code() {
        let material = material();
        let handshake_id = handshake_id();
        let mut initiator = NoiseHandshake::new(HandshakeConfig { material: &material, handshake_id: &handshake_id, role: HandshakeRole::Initiator, local_id: "host", remote_id: "viewer", prologue_context: b"cabana-test", }).expect("initiator");
        let mut responder = NoiseHandshake::new(HandshakeConfig { material: &material, handshake_id: &handshake_id, role: HandshakeRole::Responder, local_id: "viewer", remote_id: "host", prologue_context: b"cabana-test", }).expect("responder");
        ...
        assert!(initiator.is_finished()); assert!(responder.is_finished());
        ...
        assert_eq!(session_initiator.keys.send_key, session_responder.keys.recv_key);
        assert_eq!(session_initiator.keys.recv_key, session_responder.keys.send_key);
        assert_eq!(session_initiator.verification_code(), session_responder.verification_code());
        let mut encryptor = session_initiator.keys.encryptor(&session_initiator.handshake_hash);
        let mut decryptor = session_responder.keys.decryptor(&session_responder.handshake_hash);
        let frame1 = encryptor.seal(b"frame-one").expect("seal1");
        let frame2 = encryptor.seal(b"frame-two").expect("seal2");
        let plain1 = decryptor.open(&frame1).expect("open1"); assert_eq!(plain1, b"frame-one");
        let plain2 = decryptor.open(&frame2).expect("open2"); assert_eq!(plain2, b"frame-two");
        let err = decryptor.open(&frame1).expect_err("replay"); assert!(matches!(err, NoiseError::Replay(_)));
    }
    ...
    fn controller_negotiates_and_transports_media() {
        ...
        let mut host = NoiseController::new(HandshakeConfig { material: &material, handshake_id: &handshake_id, role: HandshakeRole::Initiator, local_id: "host", remote_id: "viewer", prologue_context: &context, }).expect("host controller");
        let mut viewer = NoiseController::new(HandshakeConfig { material: &material, handshake_id: &handshake_id, role: HandshakeRole::Responder, local_id: "viewer", remote_id: "host", prologue_context: &context, }).expect("viewer controller");
        let msg1 = host.take_outgoing().expect("message 1"); assert!(viewer.process_incoming(&msg1).expect("process1").is_none());
        let msg2 = viewer.take_outgoing().expect("message 2"); assert!(host.process_incoming(&msg2).expect("process2").is_none());
        let msg3 = host.take_outgoing().expect("message 3"); assert!(viewer.process_incoming(&msg3).expect("process3").is_none());
        assert!(host.handshake_complete()); assert!(viewer.handshake_complete());
        assert_eq!(host.verification_code(), viewer.verification_code());
        assert_eq!(host.handshake_hash().map(hex::encode), viewer.handshake_hash().map(hex::encode));
        let payload = b"encrypted-frame"; let outbound = host.seal_media(payload).expect("seal");
        let decoded = viewer.process_incoming(&outbound).expect("decrypt").expect("plaintext"); assert_eq!(decoded, payload);
        let reply = viewer.seal_media(b"reply-frame").expect("seal reply");
        let reply_plain = host.process_incoming(&reply).expect("decrypt reply").expect("plaintext"); assert_eq!(reply_plain, b"reply-frame");
    }
}
