...
fn test_pty_echo_command() {
    ...
    cmd.arg("hello");
    let mut child = pty_pair.slave.spawn_command(cmd).expect("Failed to spawn echo");
...
fn test_pty_cat_command() {
    ...
    let mut child = pty_pair.slave.spawn_command(cmd).expect("Failed to spawn cat");
    ...
    let output_str = String::from_utf8_lossy(&output);
    assert!(output_str.contains("test input"), "Cat should echo input: {:?}", output_str);
