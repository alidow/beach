...
impl WebRTCTransport {
    ...
    pub async fn new(config: WebRTCConfig) -> Result<Self> {
        ...
        peer_connection.on_ice_connection_state_change(Box::new(move |state: RTCIceConnectionState| {
            ...
            Box::pin(async move {
                ...
                if let Some(ref logger) = logger {
                    logger.log(&format!("[WebRTC] ICE connection state changed: {:?}", state));
                }
                match state {
                    ...
                    RTCIceConnectionState::Failed => {
                        if let Some(ref logger) = logger {
                            logger.log("[WebRTC] ICE Failed - Connection could not be established");
                        }
                    ...
                    RTCIceConnectionState::Checking => {
                        if let Some(ref logger) = logger {
                            logger.log("[WebRTC] ICE Checking - Validating connection candidates");
                        }
                    }
                    _ => {}
                }
        ...
        if matches!(mode, TransportMode::Server) {
            ...
            setup_data_channel(dc.clone(), tx_in_clone, rx_out_clone, connected_for_dc, debug_logger_dc).await;
    ...
    pub async fn initiate_remote_connection(
        ...
        while !self.is_connected() {
            if start.elapsed() > std::time::Duration::from_secs(30) {
                ...
                return Err(anyhow::anyhow!("WebRTC connection timeout - check network and firewall settings"));
            }
            
            // Log status every 5 seconds
            if last_log.elapsed() > std::time::Duration::from_secs(5) {
                webrtc_log!(self, "Still waiting for WebRTC connection... ({}s elapsed)", 
        ...
        webrtc_log!(self, "P2P connection established after {}ms!", start.elapsed().as_millis());
...
async fn setup_data_channel(
    ...
    data_channel.on_open(Box::new(move || {
        ...
        Box::pin(async move {
            if let Some(ref logger) = logger {
                logger.log(&format!("[WebRTC] Data channel '{}' opened! Connection established.", label));
            }
            *connected.write().await = true;
        })
    }));
...
impl Transport for WebRTCTransport {
    ...
    async fn send(&self, data: &[u8]) -> Result<()> {
        ...
        if dc.is_some() {
            ...
            // Use control channel
            webrtc_log!(self, "Transport send (control channel): {} bytes", data.len());
    ...
    async fn initiate_webrtc_with_signaling(
        ...
        if let Ok(remote_signaling) = any_arc.downcast::<RemoteSignalingChannel>() {
            webrtc_log!(self, "Initiating WebRTC connection as {}", 
                if is_offerer { "offerer" } else { "answerer" });
            
            // Call the non-consuming version
            self.initiate_remote_connection(remote_signaling, is_offerer).await
        } else {
            Err(anyhow::anyhow!("Invalid signaling channel type - expected RemoteSignalingChannel"))
        }
    }
