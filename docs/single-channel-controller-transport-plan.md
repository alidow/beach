...
- Decisions/changes to plan:
  ...
  - Added `create_test_pair_with_handles` helper so tests can close the remote peer connection; new `webrtc_remote_drop_reports_channel_closed` validates ChannelClosed surfacing after remote teardown (for HTTP fallback/health).
  - Signaling stub now carries metadata so manager role/bearer/session_id flow to the answerer; added JWKS-backed auth + controller frame round-trip test to cover manager-as-client metadata validation.
...
- Test evidence (links to logs/artifacts for smoke/Playwright/e2e):
  ...
  - `RUN_PONG_SHOWCASE=1 PRIVATE_BEACH_MANAGER_TOKEN=DEV-MANAGER-TOKEN NEXT_PUBLIC_PRIVATE_BEACH_MANAGER_TOKEN=DEV-MANAGER-TOKEN NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=pk_test_c2hpbmluZy1zdW5iaXJkLTE1LmNsZXJrLmFjY291bnRzLmRldiQ CLERK_SECRET_KEY=sk_test_254wHGG9vuO87RJiHJrLQsMOe1O8YuNK7dcEOKnZzm BEACH_SESSION_SERVER=http://beach-road:4132 PONG_WATCHDOG_INTERVAL=10.0 direnv exec . sh -c 'cd apps/private-beach-rewrite-2 && npx playwright test --config playwright.config.ts --project=chromium tests/e2e/pong-showcase.pw.spec.ts'` — two back-to-back PASS runs (auth on) after stack rebuilds; beaches `7648899a-5d15-4c24-bd0a-87efd3d1026e` and `c3454b39-1cef-44c2-99cc-c393ed87b306` with ball-trace motion (lhs 274 coords, rhs 405) and paddle motion (agent log 15 distinct positions). Road logs showed transient `webrtc answer unavailable (404)` while the rewrite retried offers during session-graph seeding; re-offers succeeded and transport stayed on primary channel (HTTP fallback not engaged).
  ...
  - `cargo test -p beach webrtc_remote_drop_reports_channel_closed -- --nocapture` — passes (explicit remote peer close surfaces ChannelClosed for fallback/health).
  - `cargo test -p beach webrtc_chunked_sync_round_trip -- --nocapture` — passes (large >14 KiB sync payload reassembles over the single channel).
  - `cargo test -p beach webrtc_manager_metadata_and_auth_round_trip -- --nocapture` — passes (signaling stub propagates manager metadata; JWKS-backed token accepted; controller frame round-trips on single channel).
  - `cargo test -p beach webrtc_signaling_404_then_recovers -- --nocapture` — passes (simulated early /webrtc/offer 404s; attach after delay; offerer retries and recovers onto single channel, proving the transient dual-ball path was due to early 404s before session attach).
