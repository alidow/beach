...
- [ ] M3g: Subscriber/publisher clarity: each participant (host/beach-buggy/manager) has explicit `subscriber.rs`/`publisher.rs` modules listing topics and handling, to make the bus wiring easy to audit. _Next up: factor the manager rewrite bus logic into `bus/subscriber.rs` (ingest) and `bus/publisher.rs` (actions/state/health emitters), mirror the pattern in beach-buggy + host, and add a tiny IPC test that asserts the published topics list matches the declared handler list._
