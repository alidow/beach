...
5. **Cleanup Pass**
   - 🔄 Partial: dead imports pruned, docs updated, `cargo fmt` and the full `cargo test -p beach-human` suite are green.
   - 🚧 `cargo clippy -p beach-human --all-targets -- -D warnings` still trips over pre-existing debt: e.g. `collapsible_if`, `manual_ignore_case_cmp`, `redundant-pattern-matching`, and `uninlined_format_args` in `client::terminal`; `useless_vec` in `cache::terminal::cache`; `field_reassign_with_default` in `sync::terminal::sync`; plus assorted formatting tweaks in integration tests. We need a focused lint-fix pass (or a scoped allowlist) before calling Phase 4 complete.
