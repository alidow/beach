...
fn test_cr_clears_pending_wrap() {
    ...
    println!("  Cursor: row={}, col={}", grid_at_eol.cursor.row, grid_at_eol.cursor.col);
    ...
    println!("After CR:");
    println!("  Cursor: row={}, col={}", grid_after_cr.cursor.row, grid_after_cr.cursor.col);
    
    // The cursor should be at (0, 0) - same row, column 0
    assert_eq!(grid_after_cr.cursor.row, 0, "CR should not advance to next row from pending wrap");
    ...
    for col in 0..10 {
        ...
        assert!(cell.char == ' ' || cell.char == '\0', 
                "Row 1 col {} should be empty but has '{}'", col, cell.char);
    }
}