...
- **Grid height advertised as total history length** – The host sends `HostFrame::Grid { viewport_rows, cols, history_rows }` using `terminal_sync.grid().viewport_size()` and `TerminalGrid::rows()` (`apps/beach-human/src/main.rs:1769`). Earlier revisions used `TerminalGrid::dims()` (all buffered history) for the `rows` field, so the client seeded `GridRenderer::ensure_size(rows, cols)` with a massive value and `scroll_to_tail()` (`apps/beach-human/src/client/grid_renderer.rs:604`) immediately jumped past the freshly streamed tail slice.  
